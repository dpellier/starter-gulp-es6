...
then
  echo ""
  echo "You can only release when on the master branch, please move to it"
  echo ""
  exit 0
fi
...
then
    echo "Rebased on master successful."
else
    echo "Rebase failed, there are conflicts to fix. Please rebase manually then rerun the script."
    echo ""
    git rebase --abort
    exit 0
fi
